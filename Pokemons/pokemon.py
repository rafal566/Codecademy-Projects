...
# self represnets the object that is going to be created.
# So when we create object the __init__ method is called,
# and when this constructor is called, it returns object (Pokemon in this case).
# So when we say self, it refers to object that is getting returned
# from the constructor. So essentially we are assigning, for example the name,
# of the object that is getting returned from the constructor EQUAL
# to the name that got passed in

#__init__ is used to initialize newly created object. It is callled every
#   time class is instantiated. It is constructor
...
# __repr__  gives the string with info about the class
  def __repr__(self):
...
#.format() it as a string method that takes variables as an argument and
...
# super() allowes to invoke the method of an object’s parent class
# class Fire_Pokemon inherites from Pokemon
